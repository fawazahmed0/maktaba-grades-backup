var tirmizi = {};
tirmizi.arabic = "حَدَّثَنَا مُحَمَّدُ بْنُ بَشَّارٍ حَدَّثَنَا صَفْوَانُ بْنُ عِيسَى عَنْ بِشْرِ بْنِ رَافِعٍ عَنْ عَبْدِ اللَّهِ بْنِ سُلَيْمَانَ بْنِ جُنَادَةَ بْنِ أَبِي أُمَيَّةَ عَنْ أَبِيهِ عَنْ جَدِّهِ عَنْ عُبَادَةَ بْنِ الصَّامِتِ قَالَ كَانَ رَسُولُ اللَّهِ صَلَّى اللَّهُ عَلَيْهِ وَسَلَّمَ إِذَا اتَّبَعَ الْجَنَازَةَ لَمْ يَقْعُدْ حَتَّى تُوضَعَ فِي اللَّحْدِ فَعَرَضَ لَهُ حَبْرٌ فَقَالَ هَكَذَا نَصْنَعُ يَا مُحَمَّدُ قَالَ فَجَلَسَ رَسُولُ اللَّهِ صَلَّى اللَّهُ عَلَيْهِ وَسَلَّمَ وَقَالَ خَالِفُوهُمْ قَالَ أَبُو عِيسَى هَذَا حَدِيثٌ غَرِيبٌ وَبِشْرُ بْنُ رَافِعٍ لَيْسَ بِالْقَوِيِّ فِي الْحَدِيثِ";
tirmizi.urdu = "عبادہ بن صامت رضی اللہ عنہ کہتے ہیں کہ جب رسول اللہ ﷺ کسی جنازے کے ساتھ جاتے تو جب تک جنازہ لحد (بغلی قبر)میں رکھ نہ دیاجاتا،نہیں بیٹھتے۔ایک یہودی عالم نے آپ کے پاس آکرکہا: محمد!ہم بھی ایساہی کرتے ہیں،تو رسول اللہ ﷺ بیٹھنے لگ گئے اور فرمایا:'تم ان کی مخالفت کرو'۔\nامام ترمذی کہتے ہیں: ۱-یہ حدیث غریب ہے،۲-بشر بن رافع حدیث میں زیادہ قوی نہیں ہیں۔";
tirmizi.hukam = "";
tirmizi.description = `[1020] کان رسول اللّٰہ ﷺ إذا اتبع الجنازۃ
إسنادہ ضعیف/ د 3176،جہ 1545    باب ما جاء فی الجلوس قبل أن توضع
بشر بن رافع ضعیف ( د 934،3176 ) و عبد اللّٰہ بن سلیمان ضعیف ( تق : 3369) وللحدیث شواھد۔`;
tirmizi.anwaar = [215,1];