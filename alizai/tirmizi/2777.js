var tirmizi = {};
tirmizi.arabic = "حَدَّثَنَا عَلِيُّ بْنُ حُجْرٍ أَخْبَرَنَا شَرِيكٌ عَنْ أَبِي رَبِيعَةَ عَنْ ابْنِ بُرَيْدَةَ عَنْ أَبِيهِ رَفَعَهُ قَالَ يَا عَلِيُّ لَا تُتْبِعْ النَّظْرَةَ النَّظْرَةَ فَإِنَّ لَكَ الْأُولَى وَلَيْسَتْ لَكَ الْآخِرَةُ قَالَ أَبُو عِيسَى هَذَا حَدِيثٌ حَسَنٌ غَرِيبٌ لَا نَعْرِفُهُ إِلَّا مِنْ حَدِيثِ شَرِيكٍ";
tirmizi.urdu = "بریدہ رضی اللہ عنہ سے روایت ہے کہ نبی اکرمﷺ نے فرمایا:'علی! نظر کے بعد نظر نہ اٹھاؤ،کیوں کہ تمہارے لیے پہلی نظر (معاف) ہے اور دوسری (معاف)نہیں ہے ' ۱؎۔\nامام ترمذی کہتے ہیں: یہ حدیث حسن غریب ہے،ہم اسے صرف شریک کی روایت سے جانتے ہیں۔";
tirmizi.hukam = "";
tirmizi.description = `[2777] یا علي ! لا تتبع النظرۃ النظرۃ
إسنادہ ضعیف/ د 2149 باب ما جاء في نظرۃ الفجاء ۃ`;
tirmizi.anwaar = [269,1];